...
def beregn_nedbetalingstid(
    ...
    for år in range(1, 51):
        # BTC akkumuleres med vekst
        btc_akkumulert += årlig_eksport_btc
        btc_verdi = btc_akkumulert * ((1 + btc_vekst_aar) ** år if btc_vekst_aar > 0 else 1)

        total_år = årlig_besparelse + årlig_eksport_kontant + btc_verdi
        total_inntekt = total_år  # verdi for det året
