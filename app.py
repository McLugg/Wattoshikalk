...
def beregn_nedbetalingstid(
    ...
    årlig_eksport_kontant = eksport_kwh * spotpris_nok * (1 - btc_eksportandel)
    årlig_eksport_btc = eksport_kwh * spotpris_nok * btc_eksportandel
    ...
    for år in range(1, 51):
        kontantstrøm += årlig_besparelse + årlig_eksport_kontant

        # BTC fremtidig verdi beregnet korrekt per innskudd
        btc_vekstliste.append(årlig_eksport_btc)
        btc_verdi = 0
        for i, innskudd in enumerate(btc_vekstliste):
            år_igjen = år - i
            btc_verdi += innskudd * ((1 + btc_vekst_aar) ** år_igjen)

        total_inntekt = kontantstrøm + btc_verdi
        kumulativ_total.append(total_inntekt)

        if total_inntekt >= investering_nok:
            return år, kumulativ_total

    return None, kumulativ_total
...
if check_password():
    ...
    btc_eksportandel = st.sidebar.slider("Andel eksport lagret i BTC (%)", 0, 100, 100) / 100
